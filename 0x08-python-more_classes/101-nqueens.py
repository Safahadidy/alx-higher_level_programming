#!/usr/bin/python3
"""
nqueens finds all possible solutions
to the N-Queens puzzle, including
translations and reflections.

It attempts virtual backtracking without
recursion. In local tests, the process
will start to slow down visibly for N > 8
and is successful up to N = 11. However,
it may be killed if used for N > 11. Using
recursion could allow for a lighter weight
process, but it's not yet apparent to this
student how to retain a record of
which solutions are already derived using that method.

Attributes:
    N (int): The base number of queens and the
    length of the board's side in piece positions.
    candidates (list): A list of all successful
    solutions for the given number of checked columns.
"""
